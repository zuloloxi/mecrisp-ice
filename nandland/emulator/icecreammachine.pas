...
begin
  ...
  begin
    ortogether := ortogether or $0007;
    if random(2) = 1 then ortogether := ortogether or $0008;
  end;
...
begin
  assign(image, name);
  rewrite(image);
  for addr := 0 to $FFF do writeln(image, word2hex(memory[addr]));
  close(image);
end;
...
begin
  ...
  if (pc and $1000) <> 0 then begin dpush(insn); pc := rpop shr 1; end                   // Memory fetch
  else
  begin
